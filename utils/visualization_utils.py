...
def draw_mask_on_image_array(image, mask, color='red', alpha=0.4):
  ...
  # draw mask minrect =========================== 2020-09-24===========
  import cv2
  contours, hier = cv2.findContours(np.array(pil_mask), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
  ...
  for i in range(min(max_boxes_to_draw, boxes.shape[0])):
    if scores is None or scores[i] > min_score_thresh:
      ...
      if scores is None:
        ...
        detections.append((str(class_name),scores[i],(box[1]*w,box[0]*h,box[3]*w,box[2]*h)))
